...
class BloomdConnection(object):
    ...
    def __init__(self, server, timeout, attempts=3):
        """
        Create a new Bloomd Connection.

        :Parameters:
            - server: Provided as a string, either as "host" or "host:port" or
                      "host:port:udpport". Uses the default port of 8673 if
                      None is provided for tcp, and 8674 for udp.
            - timeout: The socket timeout to use.
            - attempts (optional): Maximum retry attempts on errors. Defaults
                                   to 3.
        """
        # Parse the host/port
        ...
        self.logger = logging.getLogger("pybloomd.BloomdConnection.%s.%d" %
                                        self.server)

    def _create_socket(self):
        """Create a new socket, tries to connect to the server."""
        # Connect the socket
    ...
    def send(self, cmd):
        """Send a command with out the newline to the server."""
        ...
        for attempt in xrange(self.attempts):
            try:
                ...
                self.logger.exception(
                    "Failed to send command to bloomd server! Attempt: %d"
                    % attempt)
                if e[0] in (errno.ECONNRESET, errno.ECONNREFUSED, errno.EAGAIN,
        ...
        if not sent:
            self.logger.critical(
                "Failed to send command to bloomd server after %d attempts!"
                % self.attempts)
    ...
    def readblock(self, start="START", end="END"):
        """
        Read a response block from the server.

        The servers responses are between `start` and `end` which can be
        optionally provided. Returns an array of the lines within the block.
        """
        lines = []
        first = self.read()
        if first != start:
            raise BloomdError("Did not get block start (%s)! Got '%s'!" %
                              (start, first))
        while True:
            line = self.read()
            if line == "":
    ...
    def send_and_receive(self, cmd):
        """
        Convenience wrapper around `send` and `read`.

        Sends a command, and reads the response, performing a retry if
        necessary.
        """
        done = False
        for attempt in xrange(self.attempts):
            try:
                ...
                self.logger.exception(
                    "Failed to send command to bloomd server! Attempt: %d" %
                    attempt)
                if e[0] in (errno.ECONNRESET, errno.ECONNREFUSED,
                            errno.EAGAIN, errno.EHOSTUNREACH, errno.EPIPE):
                    self.sock = self._create_socket()
                else:
                    raise
    ...
    def response_block_to_dict(self):
        """
        Convenience wrapper around `readblock`.

        Convert a block output into a dictionary by splitting on spaces,
        and using the first column as the key, and the remainder as the value.
        """
...
class BloomdClient(object):
    ...
    """Provide a client abstraction around the BloomD interface."""

    def __init__(self, servers, timeout=None, hash_keys=False):
        """
        Create a new BloomD client.

        :Parameters:
            - servers: A list of servers, which are provided as strings in
                       the "host" or "host:port".
            - timeout: (Optional) A socket timeout to use, defaults to no
                       timeout.
            - hash_keys: (Optional) Should keys be hashed before sending to
                         bloomd. Defaults to False.
        """
        if len(servers) == 0:
    ...
    def _server_connection(self, server):
        """Return a connection to a server, tries to cache connections."""
    ...
    def _get_connection(self, filter, strict=True, explicit_server=None):
        """
        Get a connection to a server which is able to service a filter.
        ...
        :Parameters:
            ...
            - explicit_server (optional) : If provided, when a filter does
                not exist and strict is False, a connection to this server is
                made. Otherwise, the server with the fewest sets is returned.
        """
    ...
    def create_filter(self, name, capacity=None, prob=None, in_memory=False,
                      server=None):
        """
        Create a new filter on the BloomD server.

        Returns a BloomdFilter to interface with it. This will return a
        BloomdFilter object attached to the filter if the filter already
        exists.

        :Parameters:
            - name : The name of the new filter
            - capacity (optional) : The initial capacity of the filter
            - prob (optional) : The inital probability of false
                                positives.
            - in_memory (optional) : If True, specified that the filter
                                     should be created in memory only.
            - server (optional) : In a multi-server environment, this forces
                                  the filter to be created on a specific
                                  server. Should be provided in the same format
                                  as initialization "host" or "host:port".
        """
    ...
    def __getitem__(self, name):
        """Get a BloomdFilter object based on the name."""
        conn = self._get_connection(name)
        return BloomdFilter(conn, name, self.hash_keys)

    def list_filters(self, prefix=None, inc_server=False):
        """
        List all the available filters across all servers.
    ...
    def flush(self):
        """Instruct all servers to flush to disk."""
        ...
        for server in self.servers:
            ...
            resp = conn.read()
            if resp != "Done":
                raise BloomdError("Got response: '%s' from '%s'" %
                                  (resp, server))
...
class BloomdFilter(object):
    ...
    def __init__(self, conn, name, hash_keys=False):
        """
        Create a new BloomdFilter object.
    ...
    def _get_key(self, key):
        """Return the key we should send to the server."""
        if self.hash_keys:
    ...
    def add(self, key):
        """
        Add a new key to the filter.

        Returns True/False if the key was added.
        """
        resp = self.conn.send_and_receive("s %s %s" %
                                          (self.name, self._get_key(key)))
        if resp in ("Yes", "No"):
            return resp == "Yes"
        raise BloomdError("Got response: %s" % resp)

    def bulk(self, keys):
        """Perform a bulk set command, adds multiple keys in the filter."""
        command = ("b %s " % self.name) + " ".join(
            [self._get_key(k) for k in keys])
    ...
    def drop(self):
        """Delete the filter from the server. This is permanent."""
        resp = self.conn.send_and_receive("drop %s" % (self.name))
        if resp != "Done":
            raise BloomdError("Got response: %s" % resp)

    def close(self):
        """Close the filter on the server."""
    ...
    def __contains__(self, key):
        """Check if the key is contained in the filter."""
        resp = self.conn.send_and_receive("c %s %s" %
                                          (self.name,
                                           self._get_key(key)))
        if resp in ("Yes", "No"):
            return resp == "Yes"
    ...
    def multi(self, keys):
        """Perform a multi command, checks for multiple keys in the filter."""
        command = ("m %s " % self.name) + " ".join(
            [self._get_key(k) for k in keys])
        resp = self.conn.send_and_receive(command)
    ...
    def __len__(self):
        """Return the count of items in the filter."""
        info = self.info()
        return int(info["size"])

    def info(self):
        """Return the info dictionary about the filter."""
        self.conn.send("info %s" % (self.name))
        return self.conn.response_block_to_dict()

    def flush(self):
        """Force the filter to flush to disk."""
        resp = self.conn.send_and_receive("flush %s" % (self.name))
        if resp != "Done":
            raise BloomdError("Got response: %s" % resp)

    def pipeline(self):
        """Create a BloomdPipeline for pipelining multiple queries."""
        return BloomdPipeline(self.conn, self.name, self.hash_keys)


class BloomdPipeline(object):

    """Provide an interface to a single Bloomd filter."""

    def __init__(self, conn, name, hash_keys=False):
        """
        Create a new BloomdPipeline object.
    ...
    def merge(self, pipeline):
        ...
        Commands from the other pipeline are appended to the commands
        of this pipeline.
        """
    ...
    def execute(self):
        ...
        All commands are sent to the server in the order issued, and responses
        are returned in appropriate order.
        """
        # Send each command
        buf = self.buf
        self.buf = []
